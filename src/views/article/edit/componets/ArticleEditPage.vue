<template>
  <PageWrapLayout class="m-container-layout mark-down">
    <el-row style="margin-bottom: 20px">
      ...
      <el-col :span="6">
        <el-button v-if="article.status==='DRAFT'" type="warning" @click="submitAsDraft">保存草稿</el-button>
...
const submit = (saveAction = 'PUBLISHED') => {
  ...
  articleAction.value = saveAction
}
...
const loadArticleDetail = (code) => {
  ...
  return articleClient.details({
    ...
    article.text = resp.content
    article.title = resp.title
    article.status = resp.status
    article.isTop = resp.is_top
    featuredImageUrl.value = resp.featured_image
    article.featuredImage = resp.featured_image
    const categories = resp.categories
    if (categories && categories.length > 0) {
      const mainCategory = categories[0]
      if (mainCategory) {
        article.category = mainCategory.id
      }
    }
  }).finally(() => {
    loading.value = false
