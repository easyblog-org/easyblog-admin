...
export default defineConfig(({mode}: ConfigEnv): UserConfig => {
    return {
        ...
        resolve: {
            ...
            // 忽略后缀名的配置选项, 添加 AccountLoginLogDrawer.vue 选项时要记得原本默认忽略的选项也要手动写入
            extensions: ['.mjs', '.js', '.ts', '.jsx', '.tsx', '.json', 'AccountLoginLogDrawer.vue'],
        },
        //启动服务配置
        server: {
            ...
            proxy: {
                ...
                '/mxnzp-api': {
                    ...
                    changeOrigin: true,
                    rewrite: (path) => path.replace(/^\/mxnzp-api/, ''),
                },
                '/open-api': {
